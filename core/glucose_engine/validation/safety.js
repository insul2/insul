/**
 * Motor de Validação e Regras de Segurança Clínica (Insul V4)
 * Padrão: ES Modules (ESM)
 */

export const SAFETY_LIMITS = {
  MIN_GLUCOSE: 20,       // mg/dL mínimo absoluto para cálculo (abaixo é emergência extrema)
...
/**
 * Valida a integridade e segurança dos dados de entrada para o cálculo do Bolus.
 * @param {Object} input Parâmetros de cálculo
 * @returns {Object} { isValid: boolean, isHypo: boolean, errors: string[], warnings: string[] }
 */
export function validateBolusInput(input) {
  ...
  const target = Number(input.target || 100);

  // 1. Validação de Parâmetros Fundamentais (Evitar divisão por zero ou nulos)
  ...
  // 2. Alerta Crítico de Hipoglicemia (< 70 mg/dL)
